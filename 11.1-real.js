...
const logFFmpeg = (...args) => {
    const timestamp = new Date().toISOString().slice(11, 19);
    const logEntry = `[${timestamp}] ${args.filter(Boolean).join(' | ')}\n`;
...
<div class="table-wrap"><table>
  <thead><tr><th>#</th><th>ไฟล์</th><th>ขนาด</th><th>ความยาว</th><th>Codec</th><th>หน้าปก</th><th>แปลงแล้ว</th><th>วันที่</th></tr></thead>
...
function renderTable(){
  ...
  document.getElementById('tbody').innerHTML=rows.map((x,i)=>{
    ...
    var thumb=x.thumb==='_failed'?'<span class="badge broken" style="padding:1px 5px;font-size:9px">Error</span>':x.thumb?'<span class="badge ok" style="padding:1px 5px;font-size:9px">มีปก</span>':'<span class="badge queue" style="padding:1px 5px;font-size:9px">ไม่มี</span>';
...
server.listen(PORT, () => {
    ...
    db.posts.forEach(x => { 
        if (x.thumb && x.thumb !== '_failed' && !fs.existsSync(path.join(THUMB_DIR, x.thumb))) { 
            delete x.thumb; 
            fixed++; 
        } 
        if (x.convFile && !fs.existsSync(path.join(VIDEO_DIR, x.convFile))) { 
            x.convFile = null; 
            fixed++; 
        } 
    });
